...
import { View, FlatList, ActivityIndicator, AsyncStorage,
  StyleSheet,
  Text,
  Image,
  TouchableOpacity
    ...
    if (authvalue !== null) {
      fetch('https://amaapi.qayad.com/api/',{
        ...
        // alert( JSON.stringify(this.state.data)  );
          ...
          this.arrayholder = responseJson[1];
         
         
          db.addContact(responseJson).then((result) => {
            console.log(result);
            this.setState({
              loading: false,
            });
...
<Text style={{ color: '#003342', fontWeight: 'normal', fontSize: 12 }} > {item.date}</Text>
<Text style={{ color: '#003342', fontWeight: 'normal', fontSize: 12 }} > {item.client_email}</Text>
          ...
          <Text style={{ width: '30%',color: '#003342', fontWeight: 'bold', fontSize: 12, }} > Client Message</Text>
         <Text style={{ width: '70%',color: '#003342', fontWeight: 'normal', fontSize: 12 }} > {item.client_msg}</Text>
  ...
  searchFilterFunction = text => {
    ...
    const newData = this.arrayholder.filter(item => {
      const itemData = `${item.client_name.toUpperCase()} ${item.client_number} ${item.date}`;
      ...
      />
       <View style={{ backgroundColor: "#fff", width: '100%', height: 40, flexDirection: 'row', marginBottom: 3 }}
        >
          ...
          <Text style={{ width: '50%', color: '#002632', alignSelf: 'center', fontWeight: 'bold', fontSize: 12, paddingLeft: 5 }} > Name /  Phone No</Text>

          <Text style={{ width: '50%', color: '#002632', alignSelf: 'center', fontWeight: 'bold', fontSize: 12 }} >  Date </Text>
 ...
 {/* <Text> {JSON.stringify(this.state.data)}</Text>   */}
  {this.state.loading ? <ActivityIndicator style={{position: 'absolute', left: 0,
  right: 0,
  top: 0,
  bottom: 0,
  alignItems: 'center',
  justifyContent: 'center'}} size="large" color="#003342" /> : (
      <FlatList
